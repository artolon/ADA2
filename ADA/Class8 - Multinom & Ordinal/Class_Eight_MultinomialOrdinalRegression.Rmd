...
attach(Complete) #All variables from this point forward automatically come from the "complete" dataset 
  #When you're done with that, you can use the function "detach"; you can also individually call other datasets, if you need

##Re-leveling data, choose stage IV as reference
Stage_cat_re <- relevel(Stage_cat, ref = "IV")

mod <- multinom(Stage_cat_re ~ Insur_cat) #univariate multinomial logistic regression 
summary(mod)
```
...
# 2-tailed Z test
p <- (1 - pnorm(abs(z), 0, 1))*2 #we want a 2-tailed test, so multiply by 2!
p
...
```{r, echo=TRUE,  warning=FALSE}
mod1 <- polr(Stage_cat ~ Insur_cat,Hess=TRUE) #polr function for ordinal regression; HESS just gives you more information about the metrics 

summary(mod1) #Now, we only have one set of coefficients (but it has 3 meanings)
```
...
ci <- confint(mod1) #our cut off should be 0, since we haven't exponentiated yet
ci
```
...
```{r}
## odds ratios and 95%CIs
exp(cbind("Odds ratio" = -coef(mod1))) #all we have to do is add "-" to switch the order 
```
...
## Testing the parallel regression assumption using Brant Test

- Published by Rollin Brant in 1990.
- Brant Test assesses whether the observed deviations from the Ordinal Logistic Regression model are larger than what could be attributed to chance alone.
- In conjunction with polr function.
- If the probability is greater than alpha level (0.05), then the dataset satisfies the proportional odds assumption.

```{r message=FALSE, warning=FALSE}
#Univariate model
brant(mod1) #The function "brant" tests the most important assumption of logistic regression 
...
#patients with private insurance are more likely to be diagnosed at lower stages, when compared to patients with Medicaid
```
