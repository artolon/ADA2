...
ggplot(data = midus, aes(x = life_satisfaction, y = self_esteem)) +
  ...
  labs(x = "Life Satisfaction",
       ...
       subtitle = "Plot 1") #this one does ALL the data together
...
ggplot(data = midus, aes(x = life_satisfaction, y = self_esteem)) +
  ...
  labs(x = "Life Satisfaction",
       y = "Self-Esteem",
       title = "Changing Aesthetics",
       subtitle = "Plot 2") #This one does it by sex/age
...
ggplot(data = midus, aes(x = life_satisfaction, y = self_esteem)) +
  ...
  labs(x = "Life Satisfaction",
       y = "Self-Esteem",
       title = "Changing Aesthetics",
       subtitle = "Plot 3") #alpha = transparency of the points 
...
ggplot(data = midus, aes(x = heart_self)) +
  geom_bar(aes(fill = heart_self)) #we don't need to specify "y" because that will just be a count of "x"; we use fill for the bar graph to fill in the bar; use "color" to outline your bar graph 

##This will automatically put the bars in descending order 
...
# look at levels
levels(midus$heart_self) #the lower level will appear first

# make a new variable for the new order of levels, then 
# assign that new order
new_heart_self <- c("Yes", "No") #Here, we are re-leveling a factor
midus$heart_self <- factor(midus$heart_self, levels = new_heart_self) #keeping all the data thte same, and just changing the order in which R reads the levels; putting the re-leveled variable over the older variable
...
display.brewer.all() #to display your color options
display.brewer.pal(5, "Dark2") #will give us just the first 5 colors from the "dark2" palette
...
ggplot(data = midus, aes(x = heart_father, y = life_satisfaction)) +
  ...
  labs(x = "Dad Heart Attack", y = "Life Satisfaction",
       title = "Void Theme") +
  theme_void() #this theme can be helpful with GIS maps
...
display.brewer.all()
display.brewer.all(n = 3, type = "qual", colorblindFriendly = T)
#2. I like the 1st one best, so I'll go with that!
ggplot(data = midus, aes(x = hostility)) +
 geom_density(aes(fill = age_category), alpha = .9) + #increased alpha to make it easier to read
 labs(x = "Hostility",
      title = "Exercise 2") +
...
ggplot(data = midus, aes(x = heart_father, y = life_satisfaction)) +
  ...
  labs(x = "Dad Heart Attack", y = "Life Satisfaction",
       title = "Black and White Theme") +
  theme_bw() +
  theme(panel.grid.major =  element_line(color = NA), #removing grid lines
        legend.title = element_blank(), #removing the title
        plot.title = element_text(hjust = 0.5), #columns go from 0 to 1, with .5 as the center
        plot.subtitle = element_text(hjust = 0.5),
        legend.background = element_rect(color = "black", #outline of legend
                                         fill = "gray")) #fill of legend
...
ggplot(data = midus, aes(x = self_esteem)) +
  ...
  labs(x = "Self-Esteem", y = "Density", title = "Coord Change",
       subtitle = "Default") +
  theme_classic() +
  xlim(c(0,75)) #this changes the scale of your variables "change x limits" (can do the same thing with ylim, which is "change y limits")
...
ggplot(data = midus, aes(x = age_category)) +
  ...
  theme_light() +
  theme(axis.text.x = element_text(angle = 90, vjust = .5)) #this time, we're also centering it with vjust
...
ggplot(data = midus, aes(x = age_category)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = .5,
                                   hjust = 1)) #thsi time, vertical AND horizontal alignment 
...
#### `facet_grid` (facets things like a plot)
...
right-hand side is columns and left is row; tilde means "by"
...
ggplot(data = midus, aes(x = BMI, y = life_satisfaction)) +
  ...
  facet_grid(~ age_category) #"facet by age category"
...
ggplot(data = midus, aes(x = BMI, y = life_satisfaction)) +
  ...
  facet_grid(~ age_category, scales = "free_x") #so that we can free up the scale and allow y to vary
...
ggplot(data = midus, aes(x = BMI, y = life_satisfaction)) +
  ...
  facet_grid(age_category ~.) #faceting by age, but displaying as rows, rather than columns
...
ggplot(data = midus, aes(x = hostility, y = life_satisfaction)) +
  ...
  facet_grid(session ~ age_category) #session by age (2x3)
...
ggplot(data = midus, aes(x = hostility, y = life_satisfaction)) +
  ...
  facet_grid(age_category ~ session) #age by session (3x2)
```
...
Does every type of combination until it runs our of possible combinations
...
ggplot(data = midus, aes(x = hostility, y = life_satisfaction)) +
  ...
  labs(x = "Life Satisfaction", y = "Hostility",
       title = "Facet_Wrap: Plot 1") +
  facet_wrap(~ age_category, ncol = 1) #now, with only one column (so it will add rows)
...
# First, create your plots! If you want to have a single legend for every plot (ex: the colors are the same for all plots), then make sure your plots are coded accordingly. Then, ggarrange (we are assigning each one as an object)
...
#we are nesting one ggarange function into another ggarrange function 
...
meansLength <- midus %>%
  ...
  summarize(xint = mean(life_satisfaction)) #x intercept is the mean
...
ggplot(data = midus, aes(x = hostility, y = self_esteem)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) + #here, we remove the standard error ("se")
  theme_classic() +
  labs(title = "Regression Plot 1b", subtitle = "Simple Linear",
       x = "Hostility", y = "Self Esteem")

#loess = localized regression (allows you to better detect non-linear areas of the data)
...
ggplot(data = midus, aes(x = scale(midus$self_esteem),
  ...
  theme_classic() +
  annotate(geom = "text", x = 1, y = -5, #the coordinates show the position where you want the text
...
#First, we have to calculate the error (mean-/+standard dev.) so that we can add our error bars
summaryStats <- midus %>%
...
ggplot(data = midus, aes(x = hostility,
                         y = self_esteem)) +
  geom_jitter(aes(color = age_category), width = 1) + #jitter is a tiny bit of variation, so plot is more readable; width is the amount of jitter
  theme_classic() +
  scale_color_brewer(palette = "Set2") +
  labs(x = "Hostility",
       y = "Self Esteem",
...
```{r adding}
#some people put all the layers all in one go; other people prefer to add one layer at a time, by assigning each individual layer to an object (i.e. "p")
p <- ggplot(data = iris)
...
# Method 1: ggsave (will save your image to your working directory/github)
