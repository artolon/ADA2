...
```{r echo=TRUE, results='hide'}
#Create the new disease variable, using TB, HIV, AIDS, Hep, and STD; observations should be assigned "1" if they have ANY of these diseases. Observations are assigned a "0" if they have NONE of these diseases. 
data <- data %>%
...
(399/6601)*100 #6.04% have TB, HIV, Hepatitis, or a STD
...
#The full sample had a prevalence of 6.04% and the complete cases sample has a prevalence of 6.72%
sum(data_cc$Disease==1, na.rm = TRUE) 
(360/5355) #6.72% had a disease at the time of incarceration 
...
summary(BoxTid_age) #The term for linearity IS significant; therefore we violate the assumption and will not include age in the model as a numeric variable; should use "AgeCat" instead.
```
...
summary(model_unadjust) #To see p-values
...
```{r echo=TRUE}
#Run the adjusted model (Include sex, race, and age)
...
summary(model_adjust) #To see p-values
...
- This model seems to not do much better at predicting cases than the unadjusted model. However, there are still ways we can compare and potentially improve model fit. 
...
#There are 4 observations deemed influential: 1918, 6414, 6475, and 6480
```
```{r echo=TRUE, results='hide'}
...
#Look at the ORs from the balanced model
odds.n.ends(model_balance) #Magnitude of relationship between IDU and Disease decreases; OR is now 4.00 95% CI (2.78, 5.80)
...
Both the balanced and full samples yielded high magnitudes of association between injection drug use (exposure) and disease (outcome). However, because the larger full sample provides a more precise estimate, results will be interpreted only for the full (unbalanced) sample. 

After adjusting for age, race, and sex, those who had used injection drugs had 5.26 greater odds of having an infectious disease (HIV/AIDS, hepatitis, tuberculosis, and/or a sexually transmitted disease), when compared to those who had never used injection drugs (OR=5.26; 95% CI 4.14, 6.71). It should be noted, however, that the computed model had a sensitivity of 0% (correctly predicting cases) and a specificity of 100% (correctly predicting non-cases). Therefore, this model might not be ideal if used as a way to predict whether an incarcerated person has an infectious disease. The extremely low sensitivity is likely partially due to the very low prevalence of disease in this sample (6%). This low prevalence makes it particularly difficult to detect cases. 
